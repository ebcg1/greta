zero_inflated_lognormal_distribution <- R6Class(
  "zero_inflated_lognormal_distribution",
  inherit = greta::.internals$nodes$node_classes$distribution_node,
  public = list(
    initialize = function(theta, meanlog, sdlog, dim) {
      theta <- as.greta_array(theta)
      meanlog <- as.greta_array(meanlog)
      sdlog <- as.greta_array(sdlog)
      # add the nodes as children and parameters
      dim <- check_dims(theta, meanlog, sdlog, target_dim = dim)
      self$bounds <- c(0, Inf)
      
      super$initialize("zero_inflated_lognormal", dim)
    ...
    tf_distrib = function(parameters, dag) {
      ...
      log_prob <- function(x) {
        tf$where(x == 0, 
                 {tf$math$log(theta)}, 
                 {tf$math$log((1-theta)*(exp(-tf$pow(((tf$math$log(x))-meanlog),2)/(fl(2)*tf$pow(sdlog,2)))/(x*sdlog*tf$math$sqrt(fl(2)*tf$cast(tf$constant(pi), tf$float64)))))
                 })
      }
...
zero_inflated_gamma_distribution <- R6Class(
  "zero_inflated_gamma_distribution",
  inherit = greta::.internals$nodes$node_classes$distribution_node,
  public = list(
    initialize = function(theta, shape, rate, dim, truncation) {
      theta <- as.greta_array(theta)
      shape <- as.greta_array(shape)
      rate <- as.greta_array(rate)
      # add the nodes as children and parameters
      dim <- check_dims(theta, shape, rate, target_dim = dim)
      self$bounds <- c(0, Inf)
      super$initialize("zero_inflated_gamma", dim, truncation)
      self$add_parameter(theta, "theta")
    ...
    tf_distrib = function(parameters, dag) {
      ...
      # shape = alpha = k
      # rate = 1/scale = 1/sigma = B
      log_prob <- function(x) {
        tf$where(x == 0, 
                 {tf$math$log(theta)}, 
                 {tf$math$log((1-theta)*(tf$math$pow(x, shape - 1)*tf$math$exp(-rate*x)*tf$math$pow(rate, shape))/tf$exp(tf$math$lgamma(shape)))
                 })
      }
...
#' @rdname distributions
#' @export
zero_inflated_lognormal <- function(theta, meanlog, sdlog, dim = NULL) {
  distrib("zero_inflated_lognormal", theta, meanlog, sdlog, dim)
}

#' @rdname distributions
#' @export
zero_inflated_gamma <- function(theta, shape, rate, dim = NULL,truncation = c(0, Inf)) {
  distrib("zero_inflated_gamma", theta, shape, rate, dim, truncation)
}
